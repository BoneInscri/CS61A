...
(define test1 (weighted-pairs integers integers (lambda (x) (+ (car x) (cadr x)))))
(display-stream-cnt test1 30)

(define test2-tmp (weighted-pairs integers integers (lambda (x) (+ (* 2 (car x)) (* 3 (cadr x)) (* 5 (car x) (cadr x)) ) )))

(define (not-divisble x y)
  (not (= (remainder x y) 0)))
...
(define test2 (stream-filter filter-cond test2-tmp))
(display-stream-cnt test2 30)
